...
function M.track_changes(buffer, initial_lines, callback)
    debug("Starting to track changes for buffer " .. tostring(buffer))
    debug({ initial_lines = initial_lines })
    ...
    debug({ writing_pref = initial_lines })
    local prev_lines_global = initial_lines

    local function diff_to_delta(diff, prev_lines)
    ...
    local function line_change(curr_lines, first_line, last_line, new_last_line)
        ...
        debug({ curr_lines = curr_lines, prev_lines = prev_lines, prev_lines_global = prev_lines_global })
        ...
        local delta = diff_to_delta(diff, prev_lines)
        ...
        --debug({ buffer = buffer, final_delta = delta })
    ...
    local function on_lines(
        ...
        debug({ prev_lines_global = prev_lines_global })
